...
function HotBookCard({ book }) {
  ...
  const discount = isSold ? null : calcDiscountPercent(book);
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm overflow-hidden h-full flex flex-col">
      <Link to={`/sach/${bookPathSlug(book)}`} className="block shrink-0">
        <div className="aspect-[3/4] bg-green-50 relative">
          {book.image ? (
            <img
              src={book.image}
              alt={book.title}
          ...
          {isSold && (
            <div className="absolute inset-0 bg-black/40 flex items-center justify-center">
              <span className="bg-red-600 text-white text-[10px] sm:text-xs font-bold px-2 py-0.5 rounded-full shadow-lg">
                Đã bán
              </span>
            </div>
          )}
      ...
      <div className="p-2 sm:p-3 flex-1 flex flex-col min-h-0">
        ...
        {isSold ? (
          <div className="mt-2 text-center text-gray-400 text-xs py-1.5 shrink-0 italic">
            Sách đã bán
          </div>
        ) : (
