...
export default function HomePage() {
    ...
    () =>
      topCategories.map((cat) => {
        const categoryBooks = (books || []).filter((b) => {
          const cid = typeof b.categoryId === 'object' ? b.categoryId?._id : b.categoryId;
          return cid === cat._id;
        });
  ...
  const getActivePromotionForBook = (bookId) =>
    activePromotions.find((p) =>
      (p.bookIds || []).some((b) => (typeof b === 'object' ? b._id : b) === bookId)
    );
  ...
  const getBookPriceInfo = (book) => {
    ...
    if (promo?.type === 'percent') salePrice = Math.round(basePrice * (1 - Number(promo.value || 0) / 100));
    if (promo?.type === 'fixed') salePrice = Math.max(0, basePrice - Number(promo.value || 0));
  ...
  const handleAddToCart = (book) => {
    ...
    addItem({ ...book, price: salePrice, originalPrice: originalPrice || null }, 1);
  ...
  return (
    <div className="container mx-auto px-4 py-6">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-green-100 overflow-hidden h-full flex flex-col">
              ...
              <div className="divide-y divide-green-50 flex-1 overflow-auto">
                ...
                {(!categories || categories.length === 0) && (
                  <div className="px-4 py-6 text-sm text-gray-500">Chưa có danh mục.</div>
                )}
              </div>
            </div>
          </aside>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              >
                ...
                <div className="min-w-0">
                  <p className="font-bold text-green-900">Bạn cần thanh lý sách cũ?</p>
              ...
              >
                ...
                <div className="min-w-0">
                  <p className="font-bold text-green-900">Chính sách vận chuyển</p>
                  <p className="text-sm text-gray-600 truncate">
      ...
      {categorySections.map(({ category, books: categoryBooks }) => (
        <section key={category._id} className="mb-8">
          <div className="flex items-center justify-between mb-4 gap-3">
            <div className="flex items-center gap-3 min-w-0">
              <div className="w-10 h-10 rounded-lg overflow-hidden border border-green-200 bg-green-50 shrink-0">
                {category.image ? (
                  ...
                  <div className="w-full h-full flex items-center justify-center text-green-700 text-sm">📚</div>
                )}
              </div>
            ...
            <Link
              to={`/sach?categoryId=${category._id}`}
                    ...
                    <div className="p-3 min-w-0">
                      ...
                      <div className="mt-2 min-h-[48px]">
                        ...
                        <div className="mt-0.5 flex items-center gap-2">
                          <div className={`text-sm line-through ${originalPrice != null ? 'text-gray-400' : 'text-transparent'}`}>
                            {originalPrice != null ? `${originalPrice.toLocaleString('vi-VN')}₫` : '0₫'}
                          </div>
                          {discountPercent ? (
      ...
      {activePromotions.length > 0 && promoBooks.length > 0 && (
        <section className="mb-8">
          <h2 className="text-xl font-bold text-red-600 mb-4">Sách khuyến mãi</h2>
